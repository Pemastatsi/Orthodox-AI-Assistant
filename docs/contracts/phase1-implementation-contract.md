...
1. **Safety suite stability.** The required `safety-suite-execution` CI check is green on the latest `main` commit — it is a required check, so a safety regression cannot merge — **and** the live paraphrase-fuzz suite (`tests/safety/test_20_queries_paraphrases.py`, run via the `paraphrase-live-llm` workflow) has most recently passed, recorded by an `audit_entries` row with `action='ci_safety_suite_passed'`, `resource_type='ci_check'`, and `details.passed=true` written within the last **30 days** (a freshness window, not a fixed waiting period). The sustained-usage soak that a fixed calendar window stood in for is supplied directly by criteria #2, #3, and #4, which cannot be met without real traffic over time. *Amended 2026-06-04 (founder-approved): replaces the original "passes in CI for 14 consecutive calendar days, no override." That rule gated on the deterministic offline `test_20_queries.py` — already a required per-PR and on-`main` check, so the calendar window added little regression signal — while imposing an uncompressible wait and a credentialed daily DB writer. Recorded via `scripts/record_audit_event.py safety-suite-passed`; surfaced by `scripts/exit_criteria_dashboard.py` (criterion #1).*
...
8. **Operational basics.** All runs produce a `RunTrace`; the `served_answer_count` Stripe meter has reported at least one billing period; the sensitive-log retention worker (`workers/tasks/retention_cleanup.py`) has run at least once successfully, signalled by **both** a `worker.retention.completed` log event (per `docs/contracts/observability.md`) **and** an `audit_entries` row with `action='retention_purged'` and `details.deleted_count` populated. The log event serves ops dashboards; the audit row is the executable check that `scripts/exit_criteria_dashboard.py` reads.
9. **Safety configs are real.** `config/sensitivity_keywords.yaml` and `config/pastoral_filters.yaml` carry founder-approved rules covering all expected `sensitivityPrimary` categories and `riskFlags` values, including Greek-language coverage for the patterns the Orthodox-Ethos corpus actually exercises. The CI safety-suite-execution job passes against the real (non-stub) configs. Operationally enforced by (a) a startup test that fails when either YAML's `version` still equals the stub baseline `2026-05-01.1` AND `APP_ENV='production'`, and (b) an `audit_entries` row with `action='safety_config_approved'` and `resource_type='safety_config'` written by the founder (or by the deployment pipeline acting on behalf of the founder) recording the non-stub config versions in `details`. There is no separate named-reviewer requirement — founder approval is the single signoff signal; the founder is free to solicit informal Greek-reading review without it being a contractual gate. The paraphrase-fuzz suite (`tests/safety/test_20_queries_paraphrases.py`) is a CI-blocking gate equal in standing to the canonical 20-case suite once real configs land; coverage rules per `docs/contracts/safety-config-format.md` §Paraphrase Coverage.
