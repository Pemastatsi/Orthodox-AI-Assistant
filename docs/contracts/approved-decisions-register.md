...
| D-EMB-001. BGE-M3 runtime dependency (deferred to owner) | The `bge:bge-m3` embedding candidate (REC-008, seeded inert by migration 0004) needs a runtime-dependency decision before it can run: **Option A** local weights (`FlagEmbedding`/`sentence-transformers`, ~2.3GB, no egress, GPU-favoring — the ADR-0012 reranker precedent) or **Option B** a hosted route (light image, third-party egress subject to ADR-0005 review + per-call cost). The choice is **deferred to the owner**; until ratified, no bge dependency is added, the route stays `experiment`, and `build_live_harness` / the backfill driver raise a clear ADR-referencing error for it. The **openai** candidate (`text-embedding-3-large`) runs the full benchmark today, so T-009 Phase-A is not blocked — only the bge arm is. ColBERT (REC-015) + late chunking (REC-014) gate on a BGE-M3 win, which gates on this decision. See ADR-0017. |
