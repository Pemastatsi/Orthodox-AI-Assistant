...
## Artifact Cache Key (Phase 2)

Artifacts (Tier 2–5, introduced in ADR 0013) use a separate cache key distinct from the Q&A response cache key. Same algorithm (`json.dumps` sorted keys + SHA-256), different field set.

### Artifact Cache Key Fields

| Key | Source | Required |
|---|---|---|
| `tenantId` | `Principal.tenantId` | yes |
| `artifactType` | `ArtifactRequest.artifactType` | yes |
| `evidencePacketHash` | SHA-256 of serialized `EvidencePacket` from source run | yes |
| `artifactRouteId` | Active certified `ACTIVE_ARTIFACT_ROUTE_*` route ID | yes |
| `locale` | `ArtifactRequest.locale` | yes |
| `generationOptionsHash` | SHA-256 of `json.dumps(generationOptions, sort_keys=True)` | yes |
| `tenantBrandingVersion` | `tenants.config.brandingVersion` | yes |

No additional keys are permitted. The artifact cache key does not include `queryNormalized`, `sessionHash`, `answerMode`, `promptVersion`, or `modelRouteId` — those belong to the Q&A cache key.

### Artifact Cache TTL

| Artifact category | TTL |
|---|---|
| Tier 2 visual artifacts (graphs, timelines) | 6 hours |
| Tier 3 generated documents | 6 hours |
| Audio overviews (Tier 4) | 48 hours |
| Tier 5 multimedia overlays | 24 hours (liturgical context keyed on date only) |

### Invalidation

- `evidencePacketHash` change: any corpus change that updates the evidence packet for the source run invalidates related artifact cache entries.
- `artifactRouteId` change (provider upgrade): artifacts generated by the old route do not match the new route key; old entries roll off by TTL.
- `tenantBrandingVersion` bump: export-format artifacts (PDFs, DOCX, PPTX) are invalidated when branding changes; visual artifacts (graphs, maps) are not, since branding does not affect their content.

### Billing Interaction

Cache hits on artifact keys do NOT increment `generated_artifact_count`. Only fresh artifact generations increment the meter (matches Q&A cache behavior for `served_answer_count`).
