...
| `docs/adr/0017-bge-m3-runtime-dependency.md` | proposed (owner decision deferred) | BGE-M3 runtime dependency: local weights (~2.3GB, no egress) vs hosted route (light image, egress). Both documented; pick deferred to owner. Unblocks the bge arm of the T-009 embedding benchmark; openai arm runs today. |
...
| `docs/runbooks/retrieval-eval-certification.md` | canonical | Operator loop: run a candidate route's retrieval-eval → persist → attach both gates → establish baseline (owner) → certify (owner PATCH) → cutover. |
| `docs/runbooks/retrieval-eval-judge.md` | canonical | Paid Ragas-style LLM-judge run (off by default; gated by `RETRIEVAL_EVAL_RUN_JUDGE` + `deepeval` + a certified `retrieval_eval_judge` route; Anthropic Batch API). |
